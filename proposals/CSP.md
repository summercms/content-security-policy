...
We should also note that a malicious module may be completely safe in terms of the resources from the host that it uses and still cause significant harm to the user. A classic example of this would be crypto-mining WebAssembly modules that are loaded when a site is visited.
...
There is no direct equivalent of a `script` element tag for WebAssembly modules; although it is possible that such an feature may be specified in the future. Instead, WebAssembly modules are compiled and instantiated via a JavaScript API. Thus our focus on CSP for WebAssembly is on that API.
...
It is proposed that the above APIs are _gated_ by two policy points: `HostEnsureCanCompileWasmBytes` and `HostEnsureCanCompileWasmResponse`. The latter is somewhat derived from the former. 

#### `HostEnsureCanCompileWasmBytes`

If `HostEnsureCanCompileWasmBytes` is not enabled, then the `WebAssembly.compile` and `WebAssembly.compileStreaming` functions fail with a `WebAssembly.??` exception. Otherwise, these API functions return results depending on the internal integrity of the WebAssembly module being compiled.

#### `HostEnsureCanCompileWasmResponse`

If the `HostEnsureCanCompileWasmBytes` policy point is not enabled, then the `WebAssembly.instantiateStreaming` function fails with a `WebAssembly.??` exception. Otherwise, its behavior depends on the content of the `Response` object.
